...
export default function RegisterPage() {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        message.includes("подтверждения администратором") ||
        message.includes("ожидает")
      ) {
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-4 relative font-sans">
      ...
      <div className="w-full max-w-[500px] bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 p-8 md:p-10 border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          ...
          >
            {role === "volunteer" && (
              <div className="absolute top-2 right-2 text-[#1e3a8a]">
                <Check size={16} />
              </div>
            )}
            <User
              className={`mb-2 ${role === "volunteer" ? "text-[#1e3a8a]" : "text-gray-400"}`}
              size={24}
            />
            <span
          ...
          <button
            ...
            className={`relative overflow-hidden rounded-xl border-2 p-4 flex flex-col items-center text-center transition-all ${role === "employee" ? "border-[#1e3a8a] bg-blue-50/50" : "border-gray-100 hover:border-gray-200"}`}
          >
            {role === "employee" && (
              <div className="absolute top-2 right-2 text-[#1e3a8a]">
                <Check size={16} />
            ...
            <Building2
              className={`mb-2 ${role === "employee" ? "text-[#1e3a8a]" : "text-gray-400"}`}
              size={24}
            />
            <span
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              <input
                ...
                placeholder={
                  countryCode === "+7" ? "900 123 4567" : "000 000 000"
                }
          ...
          <Button
            type="submit"
            disabled={isLoading}
            className="w-full h-12 bg-[#1e3a8a] hover:bg-[#2a4ec2] text-white font-bold text-base rounded-xl mt-4 shadow-lg shadow-[#1e3a8a]/20 disabled:opacity-70"
          >
            {isLoading ? (
