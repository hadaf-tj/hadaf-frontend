...
import { fetchInstitutionById } from '@/lib/api'; // Импорт функции запроса
import { Institution } from '@/types/project';   // Импорт типов

export default function InstitutionDetailPage() {
  const params = useParams(); // Получаем ID из URL (например, '1')
  
  // 1. Состояние для данных
  ...
  // 2. Загрузка данных при изменении ID
  useEffect(() => {
    const loadDetail = async () => {
      ...
      setIsLoading(true);
      try {
        // params.id может быть массивом в некоторых случаях Next.js, приводим к строке
               ...
               <div>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span className="bg-[#ffca63] text-[#1e3a8a] px-3 py-1 rounded-lg text-sm font-bold">
                       {data.needs.filter(n => n.receivedQuantity < n.requiredQuantity).length} открыто
                    </span>
                  </div>

                  {data.needs.length === 0 ? (
                      <div className="text-center py-10 bg-white rounded-3xl border border-gray-100">
                          <p className="text-gray-500">В данный момент нужд нет.</p>
                      </div>
                  ) : (
