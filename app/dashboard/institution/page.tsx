...
export default function InstitutionDashboard() {
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center text-[#1e3a8a]">
                ...
                {needs.length === 0 ? (
                  ...
                  needs.map((item) => {
                    const percent = Math.min(
                      100,
                      Math.round(
                        (item.receivedQuantity / item.requiredQuantity) * 100,
                      ),
                    );
                    const bookedPercent = Math.min(
                      100 - percent,
                      Math.round(
                        ((item.bookedQuantity || 0) / item.requiredQuantity) *
                          100,
                      ),
                    );
                    const isDone =
                      ...
                      >
                        ...
                        <td className="py-5 px-6">
                          {isEditing ? (
                              ...
                              <div className="flex gap-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditForm({
                                      ...editForm,
                                      required_qty: Number(e.target.value),
                                    })
                                  }
                            ...
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDone ? "bg-green-100 text-green-600" : "bg-blue-50 text-[#1e3a8a]"}`}
                              >
                                {isDone ? (
                        ...
                        <td className="py-5 px-6 w-1/4">
                          ...
                          <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden flex">
                            <div
                              className={`h-full ${isDone ? "bg-green-500" : "bg-[#1e3a8a]"}`}
                              style={{ width: `${percent}%` }}
                            ></div>
                            {bookedPercent > 0 && (
                              <div
                                className="h-full bg-[#ffca63]"
                  ...
                  bookings.map((booking) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-5 px-6 text-center font-bold text-gray-900">
                          {booking.quantity}
                        </td>
                        <td className="py-5 px-6 text-gray-500 text-sm max-w-[200px] truncate">
                          {booking.note || "—"}
                        </td>
                        <td className="py-5 px-6 text-center">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-black uppercase ${st.cls}`}
                          >
                            {st.text}
                          </span>
                        </td>
