...
export default function DashboardLayout({
  ...
  const menuItems =
    user?.role === "employee" ? INSTITUTION_MENU : VOLUNTEER_MENU;
    ...
    <div className="min-h-screen bg-[#f8fafc] flex font-sans">
      ...
      >
        ...
        >
          <Link
            href="/"
            className="flex items-center gap-3 font-black text-lg tracking-tight hover:opacity-80 transition-opacity"
          >
            <div className="w-9 h-9 bg-white text-[#1e3a8a] rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg">
              <HeartHandshake size={18} />
            </div>
            {!isSidebarCollapsed && (
              <span className="whitespace-nowrap">Ҳадаф</span>
            )}
          </Link>
        </div>
        ...
        {/* Navigation */}
        <nav
          className={cn("flex-1 py-3", isSidebarCollapsed ? "px-2" : "px-3")}
        >
      ...
      >
        ...
        <header className="h-14 sm:h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30 shadow-sm">
          <button
            ...
            className="lg:hidden text-gray-500 hover:text-[#1e3a8a] -ml-1"
          >
            <Menu size={22} />
          </button>
